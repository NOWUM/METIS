...
"""
This action imports Pmax values for a list of transmissions in the current context.
Values must be defined in the changeTransmissionsPmax.csv file in the client/Standard/data folder.
...
def onExecute(action, event):  
    changeTransmissionsPmax(action, event)

def changeTransmissionsPmax(action, event):
    # Context retrieval -------------------------------------
    ...
    #open csvfile
    csvFileName = Constantes.REP_DATA + 'changeTransmissionsPmax.csv'
    CrystalOptionDialog.showConfirmDialog(None, "Changing transmission pmax according to data from " + csvFileName +
    ...
    with open(csvFileName, 'rb') as csvfile:
        ...
        for asset in getAssets(sourceContext, includedTechnologies = ["Transmission"]):
