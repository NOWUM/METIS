...
This view shows a time series of the consumption of the selected transmission asset as well as the available consumption capacity
...
ViewLabel = "Transmission consumption view (W)"


##############################
# INDEX DEFINITION & RANKING
##############################
availableCapacityIndex = 'Available consumption capacity'
stackIndexesRanking = {ELECTRICITY:1, SYNC_RESERVE_DOWN:2, MFRR_DOWN:3} #Reserve down is limited by consumption, reserve up by production

##############################
# JYTHON FUNCTIONS
##############################
def get_indexing(context):
	selectedData = [energy.getName() for energy in getEnergies(context, includedEnergies = [ELECTRICITY, SYNC_RESERVE_DOWN, MFRR_DOWN, GAS])] + [availableCapacityIndex]
	return [
...
def getData(view) :
	...
	selectedEnergies = getEnergies(context, includedEnergies = [ELECTRICITY, SYNC_RESERVE_DOWN, MFRR_DOWN, GAS])
			...
			for asset in selectedAssetsByScope[scope]:
				...
				if view.filter(scope, testCase, assetName):
					...
					for energy in getConsumedEnergies(context, asset):
						if energy in selectedEnergies:
							energyName = energy.getName()
							ret += [[[scope, testCase, assetName, energyName], results.getConsumption(asset, energy) * MW_TO_W_CONVERSION]]
					# Available capacity
...
def indexCompare(numIndex,indexValue1,indexValue2):
	...
	"""
	if indexValue1 in stackIndexesRanking and indexValue2 in stackIndexesRanking:
...
def colorizer(curve_label): 
	...
	elif MFRR_DOWN.lower() in curve_label.lower():
		return Color(133,182,134)
	elif SYNC_RESERVE_DOWN.lower() in curve_label.lower():
