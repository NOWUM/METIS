...
def getData(view) :
	...
	demandDictByZone = getZoneDictFromDpDict(context, demandDict, 1)
	
	ret = []
	totalDemandDict = defaultdict(lambda: getZeroTs(context))
	for (scope, deliveryPoint, energyName, testCase, technology, assetName) in demandDictByZone:
		if scope in selectedScopes and deliveryPoint in selectedDeliveryPoints and energyName in selectedEnergyNames and testCase in selectedTestCases: #Nec ?
			...
			demandTs = demandDictByZone[index] * MW_TO_W_CONVERSION
