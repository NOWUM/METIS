...
clusterAssetList = [F_CCGT_FLEET,F_COAL_FLEET,F_HYDRO_FLEET,F_LIGNITE_FLEET,F_OCGT_FLEET,F_OIL_FLEET,F_PUMPED_STORAGE_FLEET] if useClusterModel else []
clusterAssetList += [F_BIOMASS_FLEET] if not useResMustRunModel else []
...
mustRunAssetList += [F_SOLAR_FLEET,F_WIND_ONSHORE_FLEET,F_WIND_OFFSHORE_FLEET,F_HYDRO_ROR_FLEET,F_WASTE_FLEET,F_BIOMASS_FLEET] if useResMustRunModel else []

if useReserveModel:
	reserveAssetList = [F_NUCLEAR_FLEET,F_COAL_FLEET,F_LIGNITE_FLEET,F_CCGT_FLEET,F_OCGT_FLEET,F_OIL_FLEET,F_HYDRO_FLEET,F_PUMPED_STORAGE_FLEET,F_DSR]
	if not useResMustRunModel: # Add RES asset only if they are not in must-run
		reserveAssetList += [F_SOLAR_FLEET,F_WIND_ONSHORE_FLEET,F_WIND_OFFSHORE_FLEET,F_HYDRO_ROR_FLEET,F_WASTE_FLEET,F_BIOMASS_FLEET]
else:
	reserveAssetList = []
...
#   Apply the NTC mutiplier coeficient for transmissions
multiplyParameter(transmissionsInfos, '_pmax', NTCcorrection, [F_TRANSMISSION])
